...
resource "aws_autoscaling_group" "general_asg" {
  name = join(
    "-",
    compact([
      var.cluster_name,
      aws_launch_template.general_launch_template.name
    ])
  )
  ...
  mixed_instances_policy {
    ...
    launch_template {
      launch_template_specification {
        launch_template_id = aws_launch_template.general_launch_template.id
        version            = "$Latest"
      }
      override {
